...
typedef BOOL (WINAPI *CEP_PLUGINTYPE0)(PPLUGINTYPE0_RECORD SelectedRecord);
typedef BOOL (WINAPI *CEP_PLUGINTYPE1)(ULONG *disassembleraddress, ULONG *selected_disassembler_address, ULONG *hexviewaddress);
typedef int (WINAPI *CEP_PLUGINTYPE2)(LPDEBUG_EVENT DebugEvent);
typedef void (WINAPI *CEP_PLUGINTYPE3)(ULONG processid, ULONG peprocess, BOOL Created);
typedef void (WINAPI *CEP_PLUGINTYPE4)(int reserved);
typedef void (WINAPI *CEP_PLUGINTYPE5)(void);
typedef BOOL (WINAPI *CEP_PLUGINTYPE6ONPOPUP)(ULONG selectedAddress, char **addressofname);
typedef BOOL (WINAPI *CEP_PLUGINTYPE6)(ULONG *selectedAddress);
typedef void (WINAPI *CEP_PLUGINTYPE7)(ULONG address, char **addressStringPointer, char **bytestringpointer, char **opcodestringpointer, char **specialstringpointer, ULONG *textcolor);
typedef void (WINAPI *CEP_PLUGINTYPE8)(char **line, AutoAssemblerPhase phase);
...
//the WINAPI stuff isn't really needed since I've set compiler options to force stdcall, but this makes it clear that stdcall is used to the reader
typedef void (WINAPI *CEP_SHOWMESSAGE)(char* message);
typedef int (WINAPI *CEP_REGISTERFUNCTION) (int pluginid, PluginType functiontype, PVOID init);
typedef BOOL (WINAPI *CEP_UNREGISTERFUNCTION) (int pluginid, int functionid);
typedef HANDLE (WINAPI *CEP_GETMAINWINDOWHANDLE) (void);
typedef BOOL (WINAPI *CEP_AUTOASSEMBLE) (char *script);
typedef BOOL (WINAPI *CEP_ASSEMBLER) (UINT_PTR address, char* instruction, BYTE *output, int maxlength, int *returnedsize);
typedef BOOL (WINAPI *CEP_DISASSEMBLER) (UINT_PTR address, char* output, int maxsize);
typedef BOOL (WINAPI *CEP_CHANGEREGATADDRESS) (UINT_PTR address,PREGISTERMODIFICATIONINFO changereg);
typedef BOOL (WINAPI *CEP_INJECTDLL) (char *dllname, char *functiontocall);
typedef int (WINAPI *CEP_FREEZEMEM) (UINT_PTR address, int size);
typedef BOOL (WINAPI *CEP_UNFREEZEMEM) (int freezeID);
typedef BOOL (WINAPI *CEP_FIXMEM) (void);
typedef BOOL (WINAPI *CEP_PROCESSLIST) (char *listbuffer, int listsize);
typedef BOOL (WINAPI *CEP_RELOADSETTINGS) (void);
typedef DWORD (WINAPI *CEP_GETADDRESSFROMPOINTER) (UINT_PTR baseaddress, int offsetcount, int* offsets);  
typedef BOOL (WINAPI *CEP_GENERATEAPIHOOKSCRIPT) (char *address, char *addresstojumpto, char *addresstogetnewcalladdress, char *script, int maxscriptsize);
typedef BOOL (WINAPI *CEP_ADDRESSTONAME) (UINT_PTR address, char *name, int maxnamesize);
typedef BOOL (WINAPI *CEP_NAMETOADDRESS) (char *name, UINT_PTR *address);

typedef VOID (WINAPI *CEP_LOADDBK32)(void);
typedef BOOL (WINAPI *CEP_LOADDBVMIFNEEDED)(void);
typedef DWORD (WINAPI *CEP_PREVIOUSOPCODE)(ULONG address);
typedef DWORD (WINAPI *CEP_NEXTOPCODE)(ULONG address);
typedef BOOL (WINAPI *CEP_LOADMODULE)(char *modulepath, char *exportlist, int *maxsize);
typedef BOOL (WINAPI *CEP_DISASSEMBLEEX)(ULONG address, char *output, int maxsize);
typedef VOID (WINAPI *CEP_AA_ADDCOMMAND)(char *command);
typedef VOID (WINAPI *CEP_AA_DELCOMMAND)(char *command);

typedef PVOID (WINAPI *CEP_CREATETABLEENTRY)(void);
typedef PVOID (WINAPI *CEP_GETTABLEENTRY)(char *description);
typedef BOOL (WINAPI *CEP_MEMREC_SETDESCRIPTION)(PVOID memrec, char *description);
typedef PCHAR (WINAPI *CEP_MEMREC_GETDESCRIPTION)(PVOID memrec);
typedef BOOL (WINAPI *CEP_MEMREC_GETADDRESS)(PVOID memrec, UINT_PTR *address, DWORD *offsets, int maxoffsets, int *neededOffsets);
typedef BOOL (WINAPI *CEP_MEMREC_SETADDRESS)(PVOID memrec, char *address, DWORD *offsets, int offsetcount);
typedef int (WINAPI *CEP_MEMREC_GETTYPE)(PVOID memrec);
typedef BOOL (WINAPI *CEP_MEMREC_SETTYPE)(PVOID memrec, int vtype);
typedef BOOL (WINAPI *CEP_MEMREC_GETVALUETYPE)(PVOID memrec, char *value, int maxsize);
typedef BOOL (WINAPI *CEP_MEMREC_SETVALUETYPE)(PVOID memrec, char *value);
typedef char* (WINAPI *CEP_MEMREC_GETSCRIPT)(PVOID memrec);
typedef BOOL (WINAPI *CEP_MEMREC_SETSCRIPT)(PVOID memrec, char *script);
typedef BOOL (WINAPI *CEP_MEMREC_ISFROZEN)(PVOID memrec);
typedef BOOL (WINAPI *CEP_MEMREC_FREEZE)(PVOID memrec, int direction);
typedef BOOL (WINAPI *CEP_MEMREC_UNFREEZE)(PVOID memrec);
typedef BOOL (WINAPI *CEP_MEMREC_SETCOLOR)(PVOID memrec, DWORD color);
typedef BOOL (WINAPI *CEP_MEMREC_APPENDTOENTRY)(PVOID memrec1, PVOID memrec2);
typedef BOOL (WINAPI *CEP_MEMREC_DELETE)(PVOID memrec);

typedef DWORD (WINAPI *CEP_GETPROCESSIDFROMPROCESSNAME)(char *name);
typedef DWORD (WINAPI *CEP_OPENPROCESS)(DWORD pid);
typedef DWORD (WINAPI *CEP_DEBUGPROCESS)(int debuggerinterface);
typedef VOID (WINAPI *CEP_PAUSE)(void);
typedef VOID (WINAPI *CEP_UNPAUSE)(void);
typedef BOOL (WINAPI *CEP_DEBUG_SETBREAKPOINT)(UINT_PTR address, int size, int trigger);
typedef BOOL (WINAPI *CEP_DEBUG_REMOVEBREAKPOINT)(UINT_PTR address);
typedef BOOL (WINAPI *CEP_DEBUG_CONTINUEFROMBREAKPOINT)(int continueoption);

typedef VOID (WINAPI *CEP_CLOSECE)(void);
typedef VOID (WINAPI *CEP_HIDEALLCEWINDOWS)(void);
typedef VOID (WINAPI *CEP_UNHIDEMAINCEWINDOW)(void);

typedef PVOID (WINAPI *CEP_CREATEFORM)(void);
typedef void (WINAPI *CEP_FORM_CENTERSCREEN)(PVOID form);
typedef void (WINAPI *CEP_FORM_HIDE)(PVOID form);
typedef void (WINAPI *CEP_FORM_SHOW)(PVOID form);
typedef void (WINAPI *CEP_FORM_ONCLOSE)(PVOID form, PVOID function);

typedef PVOID (WINAPI *CEP_CREATEPANEL)(PVOID owner);
typedef PVOID (WINAPI *CEP_CREATEGROUPBOX)(PVOID owner);
typedef PVOID (WINAPI *CEP_CREATEBUTTON)(PVOID owner);
typedef PVOID (WINAPI *CEP_CREATEIMAGE)(PVOID owner);

typedef BOOL (WINAPI *CEP_IMAGE_LOADIMAGEFROMFILE)(PVOID image, char *filename);
typedef VOID (WINAPI *CEP_IMAGE_TRANSPARENT)(PVOID image, BOOL transparent);
typedef VOID (WINAPI *CEP_IMAGE_STRETCH)(PVOID image, BOOL stretch);

typedef PVOID (WINAPI *CEP_CREATELABEL)(PVOID owner);
typedef PVOID (WINAPI *CEP_CREATEEDIT)(PVOID owner);
typedef PVOID (WINAPI *CEP_CREATEMEMO)(PVOID owner);
typedef PVOID (WINAPI *CEP_CREATETIMER)(PVOID owner);

typedef VOID (WINAPI *CEP_TIMER_SETINTERVAL)(PVOID timer, int interval);
typedef VOID (WINAPI *CEP_TIMER_ONTIMER)(PVOID timer, PVOID function);

typedef VOID (WINAPI *CEP_CONTROL_SETCAPTION)(PVOID control, char *caption);
typedef BOOL (WINAPI *CEP_CONTROL_GETCAPTION)(PVOID control, char *caption, int maxsize);

typedef VOID (WINAPI *CEP_CONTROL_SETPOSITION)(PVOID control, int x, int y);
typedef int (WINAPI *CEP_CONTROL_GETX)(PVOID control);
typedef int (WINAPI *CEP_CONTROL_GETY)(PVOID control);

typedef VOID (WINAPI *CEP_CONTROL_SETSIZE)(PVOID control, int width, int height);
typedef int (WINAPI *CEP_CONTROL_GETWIDTH)(PVOID control);
typedef int (WINAPI *CEP_CONTROL_GETHEIGHT)(PVOID control);

typedef VOID (WINAPI *CEP_CONTROL_SETALIGN)(PVOID control, int align);
typedef VOID (WINAPI *CEP_CONTROL_ONCLICK)(PVOID control, PVOID function);

typedef VOID (WINAPI *CEP_OBJECT_DESTROY)(PVOID object);

typedef int (WINAPI *CEP_MESSAGEDIALOG)(char *massage, int messagetype, int buttoncombination);
typedef BOOL (WINAPI *CEP_SPEEDHACK_SETSPEED)(float speed);
...
BOOL WINAPI CEPlugin_GetVersion(PPluginVersion pv, int sizeofpluginversion);
BOOL WINAPI CEPlugin_InitializePlugin(PExportedFunctions ef, int pluginid);


#if defined(_M_AMD64) || defined(_M_IA64)
#	pragma comment(linker, "/EXPORT:CEPlugin_DisablePlugin=CEPlugin_DisablePlugin")
#	pragma comment(linker, "/EXPORT:CEPlugin_GetVersion=CEPlugin_GetVersion")
#	pragma comment(linker, "/EXPORT:CEPlugin_InitializePlugin=CEPlugin_InitializePlugin")
#else
