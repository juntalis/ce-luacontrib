...
static inline void init_funcs(struct jit* jit, lua_State* L)
{
	cfunction func;
#	define lua_pushcclosure(...) \
		get_lua_imports()->imp_lua_pushcclosure( __VA_ARGS__ )
#	define lua_pushlstring(...) \
		get_lua_imports()->imp_lua_pushlstring( __VA_ARGS__ )
#	define ADDCFUNC(NAME,FUNC,CODE) \
		lua_pushliteral(L, #NAME); \
		CODE ; \
		lua_pushcfunction(L, (lua_CFunction)(FUNC)); \
		get_lua_imports()->imp_lua_rawset(L, -3)
#	define ADDLUAFUNC(NAME) \
		ADDCFUNC(NAME,func,func = (cfunction)(get_lua_imports()->imp_##NAME))
#	define ADDFUNC(DLL,NAME) \
		ADDCFUNC(NAME,func, \
			func = DLL ? (cfunction) GetProcAddress(DLL, #NAME) : NULL; \
	...
	ADDFUNC(NULL, lua_callk);
	ADDFUNC(jit->kernel32_dll, SetLastError);
	ADDFUNC(jit->kernel32_dll, GetLastError);
	ADDLUAFUNC(luaL_error);
	ADDLUAFUNC(lua_pushnumber);
	ADDLUAFUNC(lua_pushboolean);
	ADDLUAFUNC(lua_gettop);
	ADDLUAFUNC(lua_rawgeti);
	ADDLUAFUNC(lua_pushnil);
	ADDLUAFUNC(lua_settop);
	ADDLUAFUNC(lua_remove);
#	undef ADDFUNC
#	undef ADDLUAFUNC
#	undef ADDCFUNC
